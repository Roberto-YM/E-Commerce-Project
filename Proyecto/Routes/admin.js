...
// Agregar una ruta para encontrar un usuario por correo electrónico
router.get("/find/user/:correo", async (req, res) => {
    const userEmail = req.params.correo; // Obtener el correo electrónico de la consulta

    try {
        // Buscar el usuario por su correo electrónico en la colección de usuarios
        const foundUser = await Users.findOne({ email: userEmail });

        // Si se encuentra el usuario, devolverlo como respuesta
        if(foundUser){
            return console.log(foundUser.email );
